...
func TestOpenRouterSourceFetch(t *testing.T) {
	...
	require.Len(t, offering.Exposures, 3)
	...
	assert.Equal(t, APITypeAnthropicMessages, offering.Exposures[2].APIType)
	...
	assert.Contains(t, offering.Exposures[2].SupportedParameters, ParamTools)
	...
	assert.Contains(t, offering.Exposures[2].SupportedParameters, ParamResponseFormat)
	assert.Contains(t, offering.Exposures[2].SupportedParameters, ParamTopLevelCacheControl)
	...
	assert.Contains(t, offering.Exposures[2].ParameterMappings, ParameterMapping{Normalized: ParamResponseFormat, WireName: "output_config.format"})
	assert.Contains(t, offering.Exposures[2].ParameterMappings, ParameterMapping{Normalized: ParamTopLevelCacheControl, WireName: "cache_control"})
	...
	assert.NotNil(t, offering.Exposures[2].ExposedCapabilities.Caching)
	assert.True(t, offering.Exposures[2].ExposedCapabilities.Caching.TopLevelRequestCaching)
	assert.True(t, offering.Exposures[2].ExposedCapabilities.Caching.PerMessageCaching)
	assert.True(t, offering.Exposures[2].ExposedCapabilities.StructuredOutput)
