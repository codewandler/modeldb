...
func DefaultCodexFixturePath() string { return filepath.Join("internal", "source", "codex", "testdata", "models.json") }
func (CodexSource) ID() string { return codexSourceID }

type codexReasoningLevel struct { Effort string `json:"effort"` }
...
type codexPayload struct { Models []codexModelEntry `json:"models"` }
...
func (s CodexSource) Fetch(context.Context) (*Fragment, error) {
	...
	for _, item := range payload.Models {
		if !item.SupportedInAPI { continue }
...
func capabilitiesFromCodexModel(item codexModelEntry) Capabilities {
	...
	if !strings.Contains(strings.ToLower(item.Slug), "mini") { efforts = append(efforts, ReasoningEffortNone) }
	for _, e := range item.SupportedReasoningLevels {
		switch strings.ToLower(strings.TrimSpace(e.Effort)) {
		case "low": efforts = append(efforts, ReasoningEffortLow)
		case "medium": efforts = append(efforts, ReasoningEffortMedium)
		case "high": efforts = append(efforts, ReasoningEffortHigh)
		case "max": efforts = append(efforts, ReasoningEffortMax)
		case "xhigh": efforts = append(efforts, ReasoningEffortXHigh)
		}
	}
...
func dedupeEfforts(in []ReasoningEffortLevel) []ReasoningEffortLevel {
	...
	for _, v := range in { if !seen[v] { seen[v] = true; out = append(out, v) } }
	return out
}
...
func codexSupportedParameters(item codexModelEntry) []NormalizedParameter {
	...
	if item.SupportsParallelTools { params = append(params, ParamParallelTools) }
	if len(item.SupportedReasoningLevels) > 0 { params = append(params, ParamThinking, ParamReasoningEffort) }
	if item.SupportsReasoningSummary { params = append(params, ParamReasoningSummary) }
...
func codexParameterMappings(item codexModelEntry) []ParameterMapping {
	...
	if item.SupportsParallelTools { m = append(m, ParameterMapping{Normalized: ParamParallelTools, WireName: "parallel_tool_calls"}) }
	if len(item.SupportedReasoningLevels) > 0 { m = append(m, ParameterMapping{Normalized: ParamThinking, WireName: "reasoning"}, ParameterMapping{Normalized: ParamReasoningEffort, WireName: "reasoning.effort"}) }
	if item.SupportsReasoningSummary { m = append(m, ParameterMapping{Normalized: ParamReasoningSummary, WireName: "reasoning.summary"}) }
	return m
}
...
func codexParameterValues(item codexModelEntry) map[string][]string {
	...
	if !strings.Contains(strings.ToLower(item.Slug), "mini") { efforts = append(efforts, string(ReasoningEffortNone)) }
	for _, e := range item.SupportedReasoningLevels {
		s := strings.ToLower(strings.TrimSpace(e.Effort))
		switch s { case "low", "medium", "high", "max", "xhigh": efforts = append(efforts, s) }
	}
	if len(efforts) > 0 { values[string(ParamReasoningEffort)] = efforts }
	if item.SupportsReasoningSummary { values[string(ParamReasoningSummary)] = []string{string(ReasoningSummaryAuto), string(ReasoningSummaryConcise), string(ReasoningSummaryDetailed)} }
	if item.SupportVerbosity { values["verbosity"] = []string{"low", "medium", "high"} }
